...
describe('node hl7 client - builder tests', () => {
  ...
  describe('sanity checks', () => {
    ...
    test("error - Message Object - msh 9.1 is empty ", async () => {
      try {
        // @ts-expect-error Message is not used. That's fine.
        const message = new Message({mshHeader: {
          msh_9: {
    ...
    test("error - Message Object - msh 9.3 is not 7 character long ", async () => {
      try {
        // @ts-expect-error Message is not used. That's fine.
        const message = new Message({mshHeader: {
            msh_9: {
              msh_9_1: "ADT",
              msh_9_2: "A01",
              // @ts-expect-error 9.3 should 7 characters
              msh_9_3: "ADT_A01Y"
            },
          }})
      } catch (err) {
        expect(err).toEqual(new Error('MSH.9.3 must be 7 characters in length.'))
      }
    })
