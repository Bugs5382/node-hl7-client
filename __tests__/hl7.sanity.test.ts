...
describe('node hl7 client - sanity tests', () => {
  ...
  describe('...Batch', () => {
    ...
    test('error - Batch Object - single MSH passed', async () => {
      try {
        // @ts-expect-error
        const batch = new Batch({ text: 'MSH|^~\\&|||||20081231||ADT^A01^ADT_A01|12345||2.7\rEVN||20081231' })
      } catch (err) {
        expect(err).toEqual(new Error('Unable to process a single MSH as a batch. Use Message.'))
      }
    })
